module ScoreCard
  ...
  def final_scorecard(total,over,team)
    run_rate = lambda{ |total, over| (total.to_f/over.to_f).to_f.round(2)}
    puts " \n Team #{team} requrires Runs: #{total} in #{over} overs"
    puts " Required Run rate #{run_rate.call(total,over)} per over"
  end

  def show_over_details(balls)
    ball_proc = Proc.new{|balls|
      new_balls = balls.to_i
